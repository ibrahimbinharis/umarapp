...
function renderRiwayatPage(page = 1) {
    ...
    let history = allData.filter(d => d.__type === 'setoran' || d.__type === 'ujian');
    ...
    <div class="fade-in space-y-4 pb-24">
        ...
        if (item.__type === 'ujian') {
            detail = `
                <div class="font-bold text-slate-700 text-xs">${item.type || 'Ujian'}</div>
                <div class="text-[10px] text-slate-500 truncate w-32">${item.detail || '-'}</div>
            `;
            // Ujian scores are numeric, convert to Grade? Or just show Number?
            // Existing table expects Grade String.
            // Let's just show Score + Grade estimation or just Score.
            // But the col is "Nilai".
            // And styling uses gradeColor.

            const score = item.score || 0;
            const grade = score >= 90 ? 'A+' : score >= 80 ? 'A' : score >= 70 ? 'B' : 'C';
            const gradeColor = score >= 80 ? 'text-emerald-600 bg-emerald-50' : score >= 70 ? 'text-blue-600 bg-blue-50' : 'text-red-600 bg-red-50';

            return `<tr class="hover:bg-slate-50 transition"><td class="px-4 py-3"><div class="font-bold text-slate-700 text-xs">${formatDateLong(item.date, item.time || '')}</div></td><td class="px-4 py-3 font-bold text-xs">${santriName}</td><td class="px-4 py-3 text-[10px] font-bold uppercase text-slate-500">UJIAN</td><td class="px-4 py-3">${detail}</td><td class="px-4 py-3 text-center"><span class="px-2 py-1 rounded text-xs font-black ${gradeColor}">${score} (${grade})</span></td><td class="px-4 py-3 text-center"><button onclick="deleteData('${item._id}', 'ujian')" class="text-red-400"><span class="material-symbols-outlined text-lg">delete</span></button></td></tr>`;
        }
