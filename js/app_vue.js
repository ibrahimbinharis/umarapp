...
createApp({
    setup() {
        ...
        const absensi = useAbsensi(uiData, DB);
        ...
        onMounted(async () => {
            ...
            try {
                ...
                const ensureDefaultAdmin = async () => {
                    // SAFETY CHECK: Read directly from LocalStorage to avoid memory race conditions
                    const raw = localStorage.getItem('tahfidz_v34_master');
                    if (raw) {
                        const allDocs = JSON.parse(raw);
                        const hasAdmin = allDocs.some(u => u.__type === 'user' && (u.username === 'admin' || u._id === 'U-ADMIN-001'));
                        if (hasAdmin) {
                            return; // Admin exists in storage. Do nothing.
                        }
                    }
                ...
                if (localStorage.getItem(SYNC_URL_KEY)) {
                    ...
                    const p1 = DB.syncFromCloud().catch(e => {
                        ...
                        syncStatus.status = 'error';
                        syncStatus.message = 'Cloud Offline / Akses Ditolak';
                    ...
                    if (results[0].status === 'fulfilled' && results[0].value.success) {
                        syncStatus.status = 'success';
                        syncStatus.message = 'Terhubung & Sinkron';
                    }

                    loadData();
                    uiData.surahList = window.surahList; // Populate UI list
                    await ensureDefaultAdmin();
                } else {
                    syncStatus.status = 'warning';
                    syncStatus.message = 'Mode Offline (Cek URL Sync)';
                    await initSurahData();
