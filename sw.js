...
self.addEventListener("fetch", (event) => {
    // 1. Google Scripts / API calls -> Network First (Don't cache sensitive data permanently)
    if (event.request.url.includes("script.google.com")) {
        event.respondWith(
            fetch(event.request).catch(() => {
                // Return fallback if offline? Or just fail.
                // For now, let it fail or return cached snapshot if strictly needed.
